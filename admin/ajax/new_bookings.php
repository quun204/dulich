...
if (isset($_POST['get_bookings'])) {
    ...
    // ⚙️ Đổi 'booked' ➜ 'pending' để lấy đúng dữ liệu mới
    $res = select($query, [
        "%$frm_data[search]%",
        "%$frm_data[search]%",
        "%$frm_data[search]%",
        "pending", // ✅ trạng thái đúng trong database
        0
    ], 'sssss');
    ...
    if (mysqli_num_rows($res) == 0) {
        echo "<b>Không có lượt đặt phòng mới!</b>";
        exit;
    }
    ...
    while ($data = mysqli_fetch_assoc($res)) {
        ...
        <tr>
          ...
          <td>
            ...
            <b>Trả phòng:</b> $checkout<br>
            <b>Đã thanh toán:</b> $data[trans_amt] VND<br>
            <b>Ngày đặt:</b> $date
          </td>
          <td>
...
if (isset($_POST['assign_room'])) {
    ...
    // ✅ Khi assign_room, cập nhật thành 'booked'
    $values = [1, 0, $frm_data['room_no'], 'booked', $frm_data['booking_id']];

    $res = update($query, $values, 'iis si'); // sửa theo đúng kiểu dữ liệu
